...
const ProfileSeller = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (u.profile_image) setImagePreview(null); // reset preview on fresh fetch
  ...
  const validateFields = () => {
    ...
    if (editedData.phone) {
      ...
      else if (!editedData.phone.startsWith('09')) e.phone = 'Phone number must start with 09 (e.g. 09XXXXXXXXX)';
    }
  ...
  const getPhoneStatus = () => {
    ...
    return (editedData.phone.length === 11 && editedData.phone.startsWith('09')) ? 'valid' : 'invalid';
  };
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-orange-100">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                      ...
                      <>
                        <div className="relative">
                          <input type="tel" name="phone" value={editedData.phone} onChange={handleInputChange} inputMode="numeric" maxLength={11} placeholder="09XXXXXXXXX"
                            className={`w-full px-4 py-2.5 pr-12 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all ${fieldErrors.phone ? 'border-red-400 bg-red-50' : getPhoneStatus() === 'valid' ? 'border-green-400 bg-green-50' : 'border-gray-300'}`} />
                          <span className={`absolute right-3 top-3 text-xs font-semibold ${editedData.phone.length === 11 ? 'text-green-600' : 'text-gray-400'}`}>{editedData.phone.length}/11</span>
                        </div>
                        {editedData.phone.length > 0 && (
                          <div className="mt-1.5 space-y-0.5">
                            <div className={`flex items-center gap-1 text-xs ${editedData.phone.startsWith('09') ? 'text-green-600' : 'text-red-500'}`}><span>{editedData.phone.startsWith('09') ? '✓' : '✗'}</span><span>Starts with 09</span></div>
                            <div className={`flex items-center gap-1 text-xs ${editedData.phone.length === 11 ? 'text-green-600' : 'text-red-500'}`}><span>{editedData.phone.length === 11 ? '✓' : '✗'}</span><span>Exactly 11 digits ({editedData.phone.length} entered)</span></div>
                          </div>
                        )}
                ...
                {/* Seller Docs */}
                {(profileData.proof_document || profileData.valid_id_front || profileData.valid_id_back) && (
