...
// ✅ FIX: Lazy key read — read at call time, not module load time.
...
router.get("/summary", async (req, res) => {
  try {
    ...
    if (deliveredOrderIds.length > 0) {
      ...
      deliveredRefunds = approvedRefunds || [];
    }
...
// ✅ FIX: Now returns gross_revenue (before refunds) AND revenue (net, after refunds)
//         so frontend can chart gross matching the Gross Revenue stat card.
// ─────────────────────────────────────────
router.get("/sales-by-period", async (req, res) => {
  try {
    ...
    if (groupBy === "month") {
      ...
      // ✅ FIX: Track gross_revenue separately (no refund deduction)
      const buckets = monthNames.map(m => ({ month: m, gross_revenue: 0, revenue: 0, commission: 0, orders: 0 }));
      (orders || []).forEach(o => {
        const m          = new Date(o.order_date).getMonth();
        const gross      = parseFloat(o.total_amount || 0);
        const refund     = refundByOrderId[o.id];
        const revenue    = gross - (refund?.amount || 0);
        const commission = parseFloat(o.tax || 0) - (refund?.tax || 0);
        buckets[m].gross_revenue += gross;      // ✅ before refunds
        buckets[m].revenue       += revenue;    // net after refunds
      ...
      return res.json({ success: true, groupBy: "month", data: buckets.map(b => ({
        month:         b.month,
        gross_revenue: Math.max(0, Math.round(b.gross_revenue)), // ✅ new field
    ...
    (orders || []).forEach(o => {
      ...
      const revenue    = gross - (refund?.amount || 0);
      const commission = parseFloat(o.tax || 0) - (refund?.tax || 0);
      if (!weekMap[monday]) weekMap[monday] = { gross_revenue: 0, revenue: 0, commission: 0, orders: 0 };
      weekMap[monday].gross_revenue += gross;
...
// ✅ FIX: Now includes COD + shipping fee split among sellers
// ─────────────────────────────────────────
router.get("/sellers-by-payment", async (req, res) => {
  try {
    ...
    const { data: orders } = await supabase
      .from("orders")
      .select("id, total_amount, subtotal, shipping_fee, payment_method")
      .eq("order_status", "delivered")
      .in("payment_method", ["gcash", "paypal", "cod"])
      .gte("order_date", startDate)
      .lt("order_date", endDate);
    ...
    items.forEach(i => {
      const seller = productSellerMap[i.product_id];
      if (!seller) return;
      if (!orderSellerCount[i.order_id]) orderSellerCount[i.order_id] = new Set();
      orderSellerCount[i.order_id].add(seller.id);
    });
    ...
    items.forEach(i => {
      ...
      if (!sellerMap[sid])
        sellerMap[sid] = {
          ...
          gcash_orders: 0, gcash_revenue: 0,
          paypal_orders: 0, paypal_revenue: 0,
          cod_orders: 0, cod_revenue: 0,
          orders: new Set()
        };

      const subtotalRev = i.quantity * parseFloat(i.unit_price || 0);
      const sellerCount = orderSellerCount[i.order_id]?.size || 1;
      const shippingKey = `${sid}_${i.order_id}`;
      let shippingShare = 0;
      if (!shippingAdded[shippingKey]) {
        shippingShare = parseFloat(order.shipping_fee || 0) / sellerCount;
        shippingAdded[shippingKey] = true;
      }

      const rev = subtotalRev + shippingShare;

      if (order.payment_method === "gcash")  {
        if (!sellerMap[sid].orders.has(i.order_id)) sellerMap[sid].gcash_orders++;
        sellerMap[sid].gcash_revenue  += rev;
      }
      if (order.payment_method === "paypal") {
        if (!sellerMap[sid].orders.has(i.order_id)) sellerMap[sid].paypal_orders++;
        sellerMap[sid].paypal_revenue += rev;
      }
      if (order.payment_method === "cod") {
        if (!sellerMap[sid].orders.has(i.order_id)) sellerMap[sid].cod_orders++;
        sellerMap[sid].cod_revenue    += rev;
      }
      sellerMap[sid].orders.add(i.order_id);
    });
    ...
    const data = Object.values(sellerMap).map(s => ({
      ...
      cod_revenue:    s.cod_revenue.toFixed(2),
      total_orders:   s.orders.size,
      total_revenue:  (s.gcash_revenue + s.paypal_revenue + s.cod_revenue).toFixed(2),
      orders: undefined
    }));
...
// ─────────────────────────────────────────
// GET /api/admin/analytics/products-by-seller
// ─────────────────────────────────────────
router.get("/products-by-seller", async (req, res) => {
  try {
    ...
    const { data: products } = await supabase
      .from("product")
      .select(`
        id, product_name, category, brand, price, stock_quantity, sold_count, is_active,
        seller:user_id ( id, full_name, store_name, email )
      `);
    ...
    if (orderIds.length > 0) {
      ...
      (items || []).forEach(i => {
        if (!salesMap[i.product_id])
          salesMap[i.product_id] = { units_sold: 0, revenue: 0 };
        salesMap[i.product_id].units_sold += i.quantity;
        salesMap[i.product_id].revenue   += i.quantity * parseFloat(i.unit_price || 0);
      });
    }
    ...
    const data = (products || []).map(p => ({
      ...
      seller_name:   p.seller?.store_name || p.seller?.full_name || "Unknown",
      seller_email:  p.seller?.email || "—",
      units_sold:    salesMap[p.id]?.units_sold || 0,
      revenue:       (salesMap[p.id]?.revenue   || 0).toFixed(2),
    }));

    res.json({ success: true, data });
  } catch (error) {
    console.error("Error /admin/analytics/products-by-seller:", error);
    res.status(500).json({ success: false, error: error.message });
  }
});
